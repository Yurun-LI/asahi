#ifndef MULTI_HYBRID_A_STAR_H
#define MULTI_HYBRID_A_STAR_H
...
#include <cmath>
#include <limits>
#include "common/vec.h" // 假设vec类在这个头文件中

// Pose class to represent the pose in the Hybrid A* algorithm
class Pose {
public:
    vec x_y; // x and y coordinates
    ...
    bool operator==(const Pose& other) const {
        return x_y == other.x_y && theta == other.theta;
    }
};
...
struct PoseHash {
    std::size_t operator()(const Pose& pose) const {
        return std::hash<double>()(pose.x_y.x) ^ std::hash<double>()(pose.x_y.y) ^ std::hash<double>()(pose.theta);
    }
};
